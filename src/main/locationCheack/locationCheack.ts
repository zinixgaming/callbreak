...
async function locationDistanceCheck(
  tableId: string,
  userData: userIf,
  key: string,
  rediusRange: number,
  index: number = NUMERICAL.ONE,
): Promise<string> {
  ...
  try {
    ...
    if (!tableGamePlay || !roundTableData) {
      logger.info(
        'locationDistanceCheck :: Unable to get table data then after new table find',
      );
      await remTableFromQueue(key, tableId);
      const newTableId = await findAvaiableTable(key, index);
      ...
      if (newTableId) {
        ...
        const newCreateTableId = await createTable({
          gameType,
        ...
        await setupRound({
          ...
          noOfPlayer: roundTableData.noOfPlayer,
    ...
    const activePlayerUserIdArray = <string[]>[];
    if (seats?.s0?.userId != null) {
      activePlayerUserIdArray.push(seats.s0.userId);
    }
    if (seats?.s1?.userId != null) {
      activePlayerUserIdArray.push(seats.s1.userId);
    }
    if (seats?.s2?.userId != null) {
      activePlayerUserIdArray.push(seats.s2.userId);
    }
    if (seats?.s3?.userId != null) {
      activePlayerUserIdArray.push(seats.s3.userId);
    }
    ...
    const locationKey = `LocationCheck:${userData.lobbyId}`;
    if (activePlayerUserIdArray.length >= NUMERICAL.ONE) {
      for (let i = 0; i < activePlayerUserIdArray.length; i++) {
        ...
        const distanceFind: number = disctanceCalculation(
          Number(latitude),
          Number(longitude),
          Number(getUserData.latitude),
          Number(getUserData.longitude),
        );
        logger.info(tableId, 'distanceFind ::>>> ', distanceFind);

        const rangeRediusCheck = rediusRange;
        ...
        if (distanceFind < rangeRediusCheck) {
          ...
          if (newTableId) {
            ...
            logger.info(
              tableId,
              '=== locationDistanceCheck :: CREATE TABLE ===',
            );
            const newCreateTableId = await createTable({
              gameType,
              lobbyId: userData.lobbyId,
