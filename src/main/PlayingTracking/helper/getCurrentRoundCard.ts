...
async function getCurrentRoundCard(userScore: userScoreIf[], tableId : string): Promise<any> {
    try {
        let card: cardObjIf[] = [];
        for (let index = 0; index < userScore.length; index++) {
            ...
            let cardObj : cardObjIf = {
                userId: element.userId,
                card: getplayerDetail.roundCards && getplayerDetail.roundCards.length > 0  ? getplayerDetail.roundCards : []
            }
            card.push(cardObj)
        }
