...
async function rejoinOrNewGameHandler(
  rejoinOrNewGameData: signUpRequestIf,
  socket: any,
  ...
  try {
    ...
    const userProfileData = await getUser(signUpDetails.userId);
    ...
    if (signUpDetails.isAlredyPlaying) {
      ...
      const response: any = await signUpHandler(
        {data: signUpDetails},
        isRejoinOrNewGame,
        socket,
        ack,
    ...
    const msg = MESSAGES.ERROR.COMMON_ERROR;
    let nonProdMsg = '';
    const errorCode = 500;
