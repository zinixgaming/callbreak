...
async function allLobbyWiseOnlinePlayer(req: any, res: any) {
  try {
    ...
    const query = {lobbyIds: req.body.lobbyIds};
    ...
    for (let index = 0; index < lobbyIds.length; index++) {
      ...
      const getOnlinePlayer = await getOnliPlayerCountLobbyWise(
        ONLINE_PLAYER_LOBBY,
        element,
      );
      logger.info(
        'allLobbyWiseOnlinePlayer :: getOnlinePlayer  :::',
        getOnlinePlayer,
      );

      const data = {
        lobbyId: element,
        lobbyWiseOnlinePlayer:
          getOnlinePlayer == null ? NUMERICAL.ZERO : getOnlinePlayer,
      };
      onlinePlayers.push(data);
    }
    logger.info(
      'allLobbyWiseOnlinePlayer :: onlinePlayers  :::',
      onlinePlayers,
    );
