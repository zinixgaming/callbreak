...
async function getPlayerOnlineCountLobbyWise(req: any, res: any) {
  try {
    ...
    const onlinePlayerCountLobbyWise = await getOnliPlayerCountLobbyWise(
      ONLINE_PLAYER_LOBBY,
      query.lobbyId,
    );
    logger.info(
      'getPlayerOnlineCountLobbyWise : onlinePlayerCount :: ',
      onlinePlayerCountLobbyWise,
    );
    ...
    const sendObject = {
      ...
      data:
        onlinePlayerCountLobbyWise == null
          ? NUMERICAL.ZERO
          : onlinePlayerCountLobbyWise,
    };
