...
import { getRoundTableData, getTableData, getUser, setUser } from "../../gameTable/utils";
import leaveTable  from "../leaveTable";
import { userIf } from "../../interface/userSignUpIf";



async function checkOldTableExist(socket: any, signUpData: signUpRequestIf, ack?: Function) {
    const { userId } = signUpData;
    const socketId = socket.id;

    try {
        logger.info(userId, `Starting checkOldTableExist for userId : ${userId}, socket.authToken : ${socket.authToken}`, "signUpData :: ", signUpData);
        let isUserJoinOtherLobby = false;

        let userData : userIf = await getUser(userId);
        if (!userData) { return isUserJoinOtherLobby; }
        userData.authToken = socket.authToken;
        await setUser(userId, userData);

        let userDetail : userIf = await getUser(userId);
        if (!userDetail) { return isUserJoinOtherLobby; }
        ...
        if(signUpData.lobbyId !== userDetail?.lobbyId){
            ...
            if(userDetail.tableId){ 
                ...
                if (!tableData) { return isUserJoinOtherLobby; }

                const roundTableData = await getRoundTableData(userDetail.tableId, tableData.currentRound);
                if (!roundTableData) { return isUserJoinOtherLobby; }
                ...
                if(roundTableData.tableState !== TABLE_STATE.SCOREBOARD_DECLARED)
                {
                    ...
                    CommonEventEmitter.emit(EVENTS.REJOIN_POPUP_SOCKET_EVENT, {
                        ...
                        data: {
                            ...
                            button_text: [MESSAGES.ALERT_MESSAGE.BUTTON_TEXT.REJOIN, MESSAGES.ALERT_MESSAGE.BUTTON_TEXT.NEW_GAME],
                            rejoinUserData : {
