...
async function showScoreBoardWinningAmount(
  userScore: userScoreIf[],
  winner: number[],
  tableId: string,
): Promise<winnLossAmountIf[] | boolean> {
  try {
    const winningAmount: winnLossAmountIf[] = [];
    ...
    logger.info(
      tableId,
      'showScoreBoardWinningAmount : winningAmount :>> ',
      winningAmount,
    );
    ...
    logger.error(
      tableId,
      'CATCH_ERROR: showScoreBoardWinningAmount ::',
      error,
      '-',
      userScore,
      winner,
