...
if (NODE_ENV === "PRODUCTION") {
  ...
  roundStartTimerQueue = new Bull('roundStartTimer', {redis : {host : hostname, port:port, db : Number(REDIS_DB)} });
}else{
...
const roundStartTimerCancel = async (jobId: string) => {
  try {
    const job = await roundStartTimerQueue.getJob(jobId);
    logger.debug('JOB CANCELLED ROUND START TIMER  JOB ID:" ---- ', jobId);
    logger.debug('JOB CANCELLED ROUND START TIMER  JOB ID:" job ---- ', job);
    if (job) {
      await job.remove();
      commonEventEmitter.emit(ROUND_START_TIMER_CANCELLED, jobId);
    }
  } catch (e) {
    logger.error("CATCH_ERROR : roundStartTimerCancel --:", jobId, e);
  }
};

export = roundStartTimerCancel;
