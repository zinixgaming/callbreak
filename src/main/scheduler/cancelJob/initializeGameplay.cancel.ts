...
const initializeGameplayCancel = async (jobId: string) => {
  try {
    const job = await initializeGameplayQueue.getJob(jobId);
    logger.debug('JOB CANCELLED initializeGameplay  JOB ID:" ---- ', jobId);
    logger.debug('JOB CANCELLED initializeGameplay JOB ID:" job ---- ', job);
    if (job) {
      await job.remove();
...
export = initializeGameplayCancel;
