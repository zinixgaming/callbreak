...
if (NODE_ENV === "PRODUCTION") {
  ...
  roundStartTimerQueue = new Bull('sendWinOfRoundEvent', {redis : {host : hostname, port:port, db : Number(REDIS_DB)} });
} else {
...
const winOfRoundSetupTimer = async (data: winOfRoundSetupTimerIf) => {
  try {
    data = await Validator.schedulerValidator.winOfRoundSetupTimerValidator(
      data
    );
    const options = {
