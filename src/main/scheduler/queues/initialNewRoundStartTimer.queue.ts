...
if (NODE_ENV === "PRODUCTION") {
  ...
  tableSnapshotQueue = new Bull('newRoundStartTimer', {redis : {host : hostname, port:port, db : Number(REDIS_DB)} });
...
const initialNewRoundStartTimer = async (data: initialNewRoundStartTimerIf) => {
  try {
    ...
    logger.error(data.tableId, "CATCH_ERROR : initialNewRoundStartTimer :: ", data, error);
