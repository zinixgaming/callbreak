...
if (NODE_ENV === "PRODUCTION") {
  ...
  tableSnapshotQueue = new Bull('startTurnTimer', {redis : {host : hostname, port:port, db : Number(REDIS_DB)} });
}else{
...
const playerTurnTimer = async (data: playerTurnTimerIf) => {
  try {
    ...
    logger.error(data.tableData._id,"CATCH_ERROR : playerTurnTimer :: ", data, error);
