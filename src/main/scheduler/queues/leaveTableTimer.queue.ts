...
if (NODE_ENV === "PRODUCTION") {
  ...
  tableSnapshotQueue = new Bull('timeOutLeaveTable', {redis : {host : hostname, port:port, db : Number(REDIS_DB)} });
}else{
...
const leaveTableTimer = async (data: any) => {
  try {
    ...
    logger.info(data.tableId, options, "leaveTableTimer ------ " , options.delay);
    logger.info(data.tableId, "-- ");
