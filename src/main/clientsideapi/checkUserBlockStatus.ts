...
import {
  markCompletedGameStatusIf,
  multiPlayerWinnScoreIf,
...
async function checkUserBlockStatus(
  ...
  try {
    ...
    logger.info(
      tableId,
      'resData : checkUserBlockStatus :: ',
      checkUserBlockStatusData,
    );

    if (!responce || !responce.data.success || !checkUserBlockStatusData) {
      throw new Errors.InvalidInput(
        'Unable to fetch checkUserBlockStatus data',
      );
    }
    ...
    logger.error(
      tableId,
      'CATCH_ERROR : checkUserBlockStatus :>> ',
      error,
      '-',
      tablePlayerIds,
      token,
