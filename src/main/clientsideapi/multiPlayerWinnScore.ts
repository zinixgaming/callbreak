...
async function multiPlayerWinnScore(data: multiPlayerWinnScoreIf, token: string, socketId: string) {
    ...
    try {
        ...
        logger.info(data.tableId, "multiPlayerWinnScore : responce :: ==>> ", responce.data);

        const multiPlayerSubmitScoreData = responce.data.data
        logger.info(data.tableId, "resData :: multiPlayerSubmitScore :: ==>>", multiPlayerSubmitScoreData);
        ...
        logger.error(data.tableId, 'CATCH_ERROR : multi Player Winn Score:  :>> ', error, "-", data, token);
