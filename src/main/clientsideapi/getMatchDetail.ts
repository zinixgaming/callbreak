...
async function getMatchData(data: getMatchDataIf, token: string | undefined, socketId: Socket) {
    try {
        ...
        let getMatchdata = responce.data.data;
        ...
        logger.error(data.userId, 'CATCH_ERROR :  getMatchData :>> ', data, token, "-", error);
        ...
        if (error instanceof Errors.UnknownError) {
            ...
            CommonEventEmitter.emit(
                ...
                {
                    socketId: socketId,
                    data: { message: error.response.data.message ? error.response.data.message : MESSAGES.ERROR.COMMON_ERROR },
                }
            );
        }
        else {
