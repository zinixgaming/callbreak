...
async function blockUserCheck(
  ...
  try {
    ...
    if (!tableGamePlay || !roundTableData) {
      logger.info(
        'blockUserCheck :: Unable to get table data then after new table find',
      );
    ...
    const blockUserKey = `blockUserCheck:${userData.lobbyId}`;
    ...
    if (activePlayerUserIdArray.length != NUMERICAL.ZERO) {
      const isUserBlock = await checkUserBlockStatus(
        activePlayerUserIdArray,
        userData.authToken,
        userData.socketId,
        tableId,
