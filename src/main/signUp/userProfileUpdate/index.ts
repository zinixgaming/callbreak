...
async function userProfileUpdate(userDetail: userIf, socketId: string):Promise<any>{
    try {
        ...
        logger.info(userDetail.userId, "userOwnProfile  :: >> ", userOwnProfile, "Number(userDetail.latitude) :: ", Number(userDetail.latitude), "Number(userDetail.longitude) :: ", Number(userDetail.longitude));

        let latitude : string = `${NUMERICAL.ZERO}`;
        let longitude : string = `${NUMERICAL.ZERO}`;

        if(!userDetail.latitude || !userDetail.longitude || Number(userDetail.latitude) == NUMERICAL.ZERO || Number(userDetail.longitude) == NUMERICAL.ZERO ){
        ...
        const balance = userOwnProfile?.user?.winCash + userOwnProfile?.user?.cash || 0;
        ...
        let userId = await setUser(userDetail.userId, updateUserQuery);
