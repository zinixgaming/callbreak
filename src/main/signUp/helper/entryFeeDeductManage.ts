...
async function entryFeeDeductManage(tableId: string, currentRound: number, userIds: string[]) {
    try {
        ...
        logger.info("entryFeeDeductManage :: roundTableData  :: >>>", roundTableData);
        ...
        logger.info("entryFeeDeductManage ::  multiPlayerDeductEntryFeeData :: >> ", multiPlayerDeductEntryFeeData);
        ...
        logger.info(" entryFeeDeductManage ::  roundTableData.noOfPlayer :: >> ", roundTableData.noOfPlayer);

        if (isMinPlayerEntryFeeDeducted && deductedUserIds.length === Number(roundTableData.noOfPlayer)) {
            ...
            logger.info(tableId, "initializeGameplayForFirstRound :after collecting boot : users Balance", balance)
        ...
        if (notDeductedUserIds.length > NUMERICAL.ZERO) {
            ...
            for (let i = 0; i < notDeductedUserIds.length; i++) {
                const userProfile = await await getUser(notDeductedUserIds[i]) as userIf;
                if (!userProfile) throw Error('Unable to get user data');

                logger.info("Starting notDeductedUserIds  :: leaveTable  :: " + notDeductedUserIds[i]);
                ...
                await leaveTable(tableId, PLAYER_STATE.LEFT, socketObj, undefined, false);
