...
async function formatUpdatedUserBalance(userIds:string[]) : Promise<upadedBalanceIf[] | undefined> {
    try {
        const updatedWalletAmount = <upadedBalanceIf[]> [];
        for (let index = 0; index < userIds.length; index++) {
            const element = userIds[index];
            const getUsers = await getUser(element);
            const userOwnProfile = await getUserOwnProfile(getUsers.authToken, getUsers.socketId, getUsers.userId);  
  
            const updatedBalance : number = userOwnProfile?.user?.winCash + userOwnProfile?.user?.cash || 0;
        ...
        logger.error("CATCH_ERROR: formatUpdatedUserBalance ::", error, "-",userIds);
